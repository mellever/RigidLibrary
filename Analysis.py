...
class Analysis:
    ...
    def plotStresses(self, plotCir, plotVel, plotCon, plotF, plotStress, **kwargs):
        ...
        for k in range(self.N):
            # Particle circles
            if (plotCir):
                cir1 = ptch.Circle((self.conf.x[k], self.conf.y[k]), radius=self.conf.rad[k], ec=(
                    0.5, 0.5, 0.5), fc='none', linewidth=2)
            ...
            if (plotVel):
                scale = 2*np.mean(self.conf.rad)/np.sqrt(np.mean(self.conf.dx *
                                  self.conf.dx)+np.mean(self.conf.dy*self.conf.dy))
                velvect = ptch.Arrow(self.conf.x[k], self.conf.y[k], scale *
                                     self.conf.dx[k], scale*self.conf.dy[k], width=0.3, color='b')
        ...
        if (plotCon):
            for k in range(len(self.conf.I)):
                ...
                if self.verbose:
                    if (k >= self.ncon):
                        axval.text(x0+0.5*(x1-x0)-self.small*np.sin(ang), y0+0.5 *
                                   (y1-y0)+self.small*np.cos(ang), str(k), fontsize=8)
                    else:
...
#						axval.text(x0+0.5*(x1-x0),y0+0.5*(y1-y0),str(k),fontsize=8)
    ...
    def plotPebbles(self, plotCir, plotPeb, plotPebCon, plotClus, plotOver, **kwargs):
            ...
            if (plotPeb):
                ...
                for j in range(self.pebbles.game1):
                    ...
                    if(self.pebbles.pebbles[k, j] == -1):
                        ...
                        if(t == 3):
                            color = 'b'
                        cir2 = ptch.Circle((self.conf.x[k], self.conf.y[k]), radius=0.75 *
                                           self.conf.rad[k], ec=(0.5, 0.5, 0.5), fc=color, linewidth=2)
        ...
        if (plotClus):
            ...
            for k in range(len(self.pebbles.Ifull)):
                ...
                x0, x1, y0, y1 = self.conf.getConPos2(
                    self.pebbles.Ifull[k], self.pebbles.Jfull[k])
                if (self.pebbles.cluster[k] != -1):
                    if (k >= self.ncon):
                        ang = np.arctan((y1-y0)/(x1-x0))
                        conlink = lne.Line2D([x0-self.small*np.sin(ang), x1-self.small*np.sin(ang)], [y0+self.small*np.cos(
                            ang), y1+self.small*np.cos(ang)], linewidth=3, color=self.cluster_colors[self.pebbles.cluster[k].astype(int)])
                    else:
                        conlink = lne.Line2D([x0, x1], [y0, y1], linewidth=3,
                                             color=self.cluster_colors[self.pebbles.cluster[k].astype(int)])
                    axval.add_line(conlink)
                else:
                    if (k >= self.ncon):
                        ang = np.arctan((y1-y0)/(x1-x0))
                        conlink = lne.Line2D([x0-self.small*np.sin(ang), x1-self.small*np.sin(ang)], [
                                             y0+self.small*np.cos(ang), y1+self.small*np.cos(ang)], linewidth=3, color=(0.65, 0.65, 0.65))
                    else:
                        conlink = lne.Line2D([x0, x1], [y0, y1], linewidth=3,
            ...
            for k in range(len(self.pebbles.Ifull)):
                ...
                if (self.pebbles.Overcon[k] == True):
                    if (k >= self.ncon):
                        ang = np.arctan((y1-y0)/(x1-x0))
                        conlink = lne.Line2D([x0-self.small*np.sin(ang), x1-self.small*np.sin(ang)], [
                                             y0+self.small*np.cos(ang), y1+self.small*np.cos(ang)], linewidth=3, color=(0.8, 0.2, 0.2))
                    else:
                        conlink = lne.Line2D([x0, x1], [y0, y1],
                                             linewidth=3, color=(0.8, 0.2, 0.2))
                    axval.add_line(conlink)
                else:
                    if (k >= self.ncon):
                        ang = np.arctan((y1-y0)/(x1-x0))
            ...
            for k in range(len(self.pebbles.Ifull)):
                ...
                if (self.pebbles.ptr[k] != -1):
                    if (k >= self.ncon):
                        ang = np.arctan((y1-y0)/(x1-x0))
                        conlink = lne.Line2D([x0-self.small*np.sin(ang), x1-self.small*np.sin(ang)], [
                                             y0+self.small*np.cos(ang), y1+self.small*np.cos(ang)], linewidth=3, color=(0.6, 0, 0.0))
                    else:
                    ...
                    if (k >= self.ncon):
                        ang = np.arctan((y1-y0)/(x1-x0))
                        conlink = lne.Line2D([x0-self.small*np.sin(ang), x1-self.small*np.sin(ang)], [
                                             y0+self.small*np.cos(ang), y1+self.small*np.cos(ang)], linewidth=3, color=(0.5, 0.5, 0.5))
                    else:
                        conlink = lne.Line2D([x0, x1], [y0, y1],
        ...
        if self.conf.datatype == 'simulation':
            ...
            axval.set_xlim(xmin, xmin+self.Lx+120)
            axval.set_ylim(ymin, ymin+self.Ly+120)
        axval.set_title('System with ' + str(self.pebbles.freepeb) +
                        ' free pebbles and ' + str(self.pebbles.fail) + ' failed contacts')
        # returning figure pointer for use outside (like plotting or saving)
        if 'figure' in kwargs:
            return fig
        elif 'axis' in kwargs:
    ...
    def clusterStatistics(self):
        ...
        if (len(islargest) > 0):
            ...
            xval = np.sort(np.union1d(
                self.conf.x[Iarr[islargest]], self.conf.x[Jarr[islargest]]))
            ...
            yval = np.sort(np.union1d(
                self.conf.y[Iarr[islargest]], self.conf.y[Jarr[islargest]]))
    ...
    def crossCorrelate(self):
        ...
        if self.conf.hasAngles:
            ...
            dijtang = drijtan - (self.conf.rad[self.conf.I]*self.conf.dalpha[self.conf.I] +
                                 self.conf.rad[self.conf.J]*self.conf.dalpha[self.conf.J])
            ...
            dtang_not_signed = np.mean(
                dijtang[notmobi]*np.sign(self.conf.ftan[notmobi]))
    ...
    def ModeClusters(self, mode, thresh):
        ...
        for k in self.hessian.isrigid:
            cir1 = ptch.Circle((self.conf.x[k], self.conf.y[k]), radius=self.conf.rad[k], ec=(
                0.0, 0.0, 0.0), fc='none', linewidth=2)
            axval.add_patch(cir1)
        for k in self.hessian.notrigid:
            cir1 = ptch.Circle((self.conf.x[k], self.conf.y[k]), radius=self.conf.rad[k], ec=(
                0.65, 0.65, 0.65), fc='none', linewidth=2)
            axval.add_patch(cir1)
        axval.plot(self.conf.x[self.hessian.isrigid],
                   self.conf.y[self.hessian.isrigid], '.k')
    ...
    def RigidModesCorrelate(self, thresh, plotDisp=True):
        ...
        for k in range(self.ncon):
            ...
            if plotDisp:
                ...
                for k in range(self.N):
                    cir1 = ptch.Circle((self.conf.x[k], self.conf.y[k]), radius=self.conf.rad[k], ec=(
                        0.7, 0.7, 0.7), fc='none', linewidth=2)
                    axval.add_patch(cir1)
                ...
                for k in range(self.ncon):
                    ...
                    if fval < 0.2:
                        fval = 0.2
                    x0, x1, y0, y1 = self.conf.getConPos(k)
                    conlink = lne.Line2D([x0, x1], [y0, y1], lw=1.0 /
                                         fval, color=Fcolor(1.0/fval))
                    axval.add_line(conlink)
                axval.axis('scaled')
                if self.conf.datatype == 'simulation':
                    axval.set_xlim(-self.Lx/2, self.Lx/2)
    ...
    def RigidDisplacementsCorrelate(self, minThresh, plotDisp=True):
        ...
        # Compute correlations between being part of a rigid cluster and contact displacements
        if self.conf.hasAngles:
            disp2n, disp2t, disp2r, disp2gear, intThresh = self.conf.Disp2Contacts(
                minThresh, True)
        else:
        ...
        if (plotdis):
            ...
            for k in range(self.N):
                ...
                if (duse > 3.0*D2_scale):
                    duse = 3.0*D2_scale
                cir1 = ptch.Circle((self.conf.x[k], self.conf.y[k]), radius=0.5*radscale*duse /
                                   D2_scale, ec=Fcolor(duse/D2_scale), fc=Fcolor(duse/D2_scale), linewidth=2)
                axval.add_patch(cir1)
        ...
        for k in range(len(self.pebbles.Ifull)):
            ...
            if (self.pebbles.cluster[k] != -1):
                if (k >= self.ncon):
                    ang = np.arctan((y1-y0)/(x1-x0))
                    conlink = lne.Line2D([x0-self.small*np.sin(ang), x1-self.small*np.sin(ang)], [
                                         y0+self.small*np.cos(ang), y1+self.small*np.cos(ang)], linewidth=2, color=(0, 0, 1))
                else:
                ...
                if (k >= self.ncon):
                    ang = np.arctan((y1-y0)/(x1-x0))
                    conlink = lne.Line2D([x0-self.small*np.sin(ang), x1-self.small*np.sin(ang)], [
                                         y0+self.small*np.cos(ang), y1+self.small*np.cos(ang)], linewidth=2, color=(0.65, 0.65, 0.65))
                else:
                    conlink = lne.Line2D([x0, x1], [y0, y1], linewidth=2,
                                         color=(0.65, 0.65, 0.65))
                axval.add_line(conlink)
